...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication)
        ...
        MemberDetails principal = (MemberDetails) authentication.getPrincipal();//현재 사용자의 정보가 담겨있음.
        Member findMember = memberService.getById(principal.getId());//회원 ID를 추출한 후 DB에서 회원 정보를
        // 조회하는 이유는 OAuth 제공자(Google 등)가 주는 제한된 정보만으로는 충분하지 않기 때문입니다.
